...
class _LeaveManagementScreenState extends State<LeaveManagementScreen>
  ...
  int get _pendingCount  => _allLeaves.where((l) => l.status == 'pending').length;
  int get _approvedCount => _allLeaves.where((l) => l.status == 'approved').length;
  int get _rejectedCount => _allLeaves.where((l) => l.status == 'rejected').length;
  ...
  List<AdminLeaveData> get _visibleLeaves {
    return _allLeaves.where((l) {
      ...
      final matchesDept   = _deptFilter == 'all'   ||
          (l.user?.department ?? '') == _deptFilter;
  ...
  Widget _buildTopBar(bool isMob) {
    return Container(
      padding: EdgeInsets.symmetric(
          horizontal: isMob ? 14 : 20, vertical: 14),
  ...
  Widget _buildLoader() => Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 14),
          const Text('Loading leave requests...',
              style: TextStyle(color: Colors.white60, fontSize: 12)),
        ]),
      );
    ...
    return RefreshIndicator(
      ...
      child: CustomScrollView(
        ...
        slivers: [
          SliverToBoxAdapter(
            child: Padding(
              padding: EdgeInsets.fromLTRB(isMob ? 14 : 20, 18, isMob ? 14 : 20, 0),
              child: _buildStats(isMob),
            ),
          ),
          SliverToBoxAdapter(
            child: Padding(
              padding: EdgeInsets.fromLTRB(isMob ? 14 : 20, 16, isMob ? 14 : 20, 0),
              child: _buildFilters(isMob),
            ),
          ),
          SliverToBoxAdapter(
            child: Padding(
              padding: EdgeInsets.fromLTRB(isMob ? 14 : 20, 18, isMob ? 14 : 20, 32),
  ...
  Widget _buildStats(bool isMob) {
    final defs = [
      _StatDef(Icons.calendar_today_rounded,   const Color(0xFF7B3F00), Colors.orange.shade700, _totalCount,    'Total Requests'),
      _StatDef(Icons.calendar_month_rounded,    const Color(0xFF5A4000), Colors.amber.shade600,  _pendingCount,  'Pending'),
      _StatDef(Icons.check_circle_rounded,      const Color(0xFF003A1F), _green,                  _approvedCount, 'Approved'),
      _StatDef(Icons.cancel_rounded,            const Color(0xFF3A0010), _red,                    _rejectedCount, 'Rejected'),
    ];
    ...
    if (isMob) {
      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SingleChildScrollView(
          ...
          child: Row(children: [
            _filterChip('all', 'All Status', Colors.white54, _statusFilter,
                (v) => setState(() => _statusFilter = v)),
            const SizedBox(width: 6),
            _filterChip('pending', 'Pending', _orange, _statusFilter,
                (v) => setState(() => _statusFilter = v)),
            const SizedBox(width: 6),
            _filterChip('approved', 'Approved', _green, _statusFilter,
                (v) => setState(() => _statusFilter = v)),
            const SizedBox(width: 6),
            _filterChip('rejected', 'Rejected', _red, _statusFilter,
                (v) => setState(() => _statusFilter = v)),
            const SizedBox(width: 6),
            _filterChip('cancelled', 'Cancelled', Colors.white38, _statusFilter,
                (v) => setState(() => _statusFilter = v)),
          ]),
        ),
        ...
        SingleChildScrollView(
          scrollDirection: Axis.horizontal,
          child: Row(children: [
            _filterChip('all', 'All Types', Colors.white54, _typeFilter,
                (v) => setState(() => _typeFilter = v)),
            const SizedBox(width: 6),
            _filterChip('sick', 'Sick', _blue, _typeFilter,
                (v) => setState(() => _typeFilter = v)),
            const SizedBox(width: 6),
            _filterChip('paid', 'Paid', _green, _typeFilter,
                (v) => setState(() => _typeFilter = v)),
            const SizedBox(width: 6),
            _filterChip('unpaid', 'Unpaid', _orange, _typeFilter,
                (v) => setState(() => _typeFilter = v)),
          ]),
        ),
      ]);
      ...
      child: Row(children: [
        if (depts.length > 1) ...[
          Expanded(child: _dropdownFilter(
            ...
            items: depts.map((d) => PopupMenuItem<String>(
              value: d,
        ...
        Expanded(child: _dropdownFilter(
          ...
          label: _statusFilter == 'all' ? 'All Status' : _capitalize(_statusFilter),
        ...
        Expanded(child: _dropdownFilter(
          ...
          label: _typeFilter == 'all' ? 'All Types' : _leaveTypeLabel(_typeFilter),
  ...
  Widget _filterGroupLabel(String text) =>
      Text(text,
          style: const TextStyle(
              color: Colors.white54, fontSize: 10.5, fontWeight: FontWeight.w600));

  Widget _filterChip(String value, String label, Color color, String current,
      ...
      child: ConstrainedBox(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(
            ...
            child: Row(children: [
              SizedBox(width: 180, child: Text('Applicant',   style: hdStyle)),
              SizedBox(width: 120, child: Text('Department',  style: hdStyle)),
              SizedBox(width: 110, child: Text('Leave Type',  style: hdStyle)),
              SizedBox(width: 150, child: Text('Duration',    style: hdStyle)),
              SizedBox(width: 200, child: Text('Reason',      style: hdStyle)),
              SizedBox(width: 110, child: Text('Applied On',  style: hdStyle)),
  ...
  Widget _buildTableRow(AdminLeaveData leave, int index) {
    ...
    final appliedStr = DateFormat('MMM d, yyyy').format(leave.createdAt.toLocal());

    final sameDay = leave.startDate.toLocal().day   == leave.endDate.toLocal().day &&
      ...
      Container(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
          ...
          SizedBox(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(leave.user?.name ?? 'Employee',
                    style: const TextStyle(
                        color: Colors.white, fontSize: 13, fontWeight: FontWeight.w500),
                    overflow: TextOverflow.ellipsis),
                if ((leave.user?.employeeId ?? '').isNotEmpty)
                  Text(leave.user!.employeeId!,
                      style: const TextStyle(color: Colors.white38, fontSize: 11),
                      overflow: TextOverflow.ellipsis),
              ])),
            ]),
          ...
          SizedBox(
            width: 120,
            child: Row(children: [
              const Icon(Icons.business_outlined, size: 12, color: Colors.white24),
              const SizedBox(width: 4),
              Expanded(child: Text(leave.user?.department ?? 'N/A',
          ...
          SizedBox(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              if (sameDay)
                Text(startStr, style: const TextStyle(color: Colors.white70, fontSize: 12))
              else ...[
                Text(startStr, style: const TextStyle(color: Colors.white70, fontSize: 12)),
                Text('to $endStr', style: const TextStyle(color: Colors.white38, fontSize: 11)),
              ],
          ...
          SizedBox(
            ...
            child: isPending
                ? Row(children: [
                    _iconAction(icon: Icons.check_rounded, color: _green,
                        tooltip: 'Approve', onTap: () => _approveLeave(leave)),
                    const SizedBox(width: 8),
                    _iconAction(icon: Icons.close_rounded, color: _red,
                        tooltip: 'Reject', onTap: () => _rejectLeave(leave)),
                  ])
  ...
  Widget _buildMobileCard(AdminLeaveData leave) {
    ...
    final startStr     = fmt.format(leave.startDate.toLocal());
    final endStr       = fmt.format(leave.endDate.toLocal());
    final appliedStr   = DateFormat('d MMM yyyy').format(leave.createdAt.toLocal());
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Padding(
              ...
              child: Wrap(spacing: 8, runSpacing: 6, children: [
                ...
                if (leave.isHalfDay)
                  _infoBadge(icon: Icons.timelapse_rounded,
                      text: 'Â½ ${leave.session != null ? _capitalize(leave.session!) : "Day"}',
                      color: _blue),
                _infoBadge(icon: Icons.calendar_today_outlined,
                    text: sameDay ? startStr : '$startStr â€“ $endStr',
                    color: Colors.white38),
                _infoBadge(icon: Icons.timer_outlined,
                    text: leave.days == 0.5 ? '0.5 day'
                        : '${leave.days.toStringAsFixed(leave.days % 1 == 0 ? 0 : 1)} day${leave.days == 1 ? "" : "s"}',
                    color: Colors.white38),
                if ((leave.user?.department ?? '').isNotEmpty)
                  _infoBadge(icon: Icons.business_outlined,
                      text: leave.user!.department!, color: Colors.white38),
              ]),
            ),
            Padding(
              ...
              child: Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Text('Reason',
                      style: TextStyle(color: Colors.white38, fontSize: 9.5, fontWeight: FontWeight.w600)),
              ...
              Padding(
                ...
                child: Container(
                  ...
                  child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Icon(Icons.note_outlined, color: _red.withOpacity(0.6), size: 13),
                    const SizedBox(width: 6),
                    Expanded(child: Text(leave.reviewNote!,
            ...
            Container(
              ...
              child: Column(children: [
                Row(children: [
                  Icon(Icons.access_time_rounded, size: 11, color: Colors.white24),
                  const SizedBox(width: 4),
                ...
                if (isPending) ...[
                  const SizedBox(height: 10),
                  Row(children: [
                    Expanded(child: _actionBtn(label: 'Reject', icon: Icons.close_rounded,
                        color: _red, filled: false, loading: false,
                        onTap: () => _rejectLeave(leave))),
                    const SizedBox(width: 8),
                    Expanded(child: _actionBtn(label: 'Approve', icon: Icons.check_rounded,
                        color: _green, filled: true, loading: false,
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('${_leaveTypeLabel(leave.leaveType)} â€¢ ${leave.days} day(s)',
                  style: const TextStyle(color: Colors.white54, fontSize: 12)),
            ])),
          ]),
          const SizedBox(height: 14),
          ...
          const SizedBox(height: 10),
          _qBtn(icon: Icons.check_rounded, label: 'Approve Leave', color: _green,
              onTap: () { Navigator.pop(ctx); _approveLeave(leave); }),
          const SizedBox(height: 10),
          _qBtn(icon: Icons.close_rounded, label: 'Reject Leave', color: _red,
              onTap: () { Navigator.pop(ctx); _rejectLeave(leave); }),
          const SizedBox(height: 6),
        ]),
      ),
    );
              ...
              children: [
                if (loading)
                  SizedBox(width: 13, height: 13,
                      child: CircularProgressIndicator(
                          strokeWidth: 1.5, color: filled ? Colors.white : color))
                else
  ...
  Color _typeColor(String type) {
    switch (type.toLowerCase()) {
      case 'sick':   return _blue;
      case 'unpaid': return _orange;
...
class _StatDef {
  ...
  final int count;
  final String label;
  const _StatDef(this.icon, this.bgColor, this.iconColor, this.count, this.label);
}
...
class _TapScalerState extends State<_TapScaler>
  ...
  late final AnimationController _ctrl =
      AnimationController(duration: const Duration(milliseconds: 100), vsync: this, value: 1);
